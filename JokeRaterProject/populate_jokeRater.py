...
def populate():
    ...
    joke.save()
    joke = Joke(category=science,content="I would tell you a chemistry joke...",punchline="...but all the good ones Ar-gon",rating=random.randint(-20, 20),postingUser=bob,datePosted=datetime.datetime.now().date()- timedelta(days=random.randint(0, 10)),reportFlag=False)
    joke.save()
    joke = Joke(category=science,content="What do you call a clown who's in jail? ",punchline="A sili-con",rating=random.randint(-20, 20),postingUser=Tim,datePosted=datetime.datetime.now().date()- timedelta(days=random.randint(0, 10)),reportFlag=False)
    joke.save()
